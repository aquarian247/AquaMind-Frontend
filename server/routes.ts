...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/health/records", async (req, res) => {
    try {
      ...
      const records = batchId ? allRecords.filter(r => r.batch === batchId) : allRecords;
      res.json(records);
    } catch (error) {
  ...
  app.get("/api/v1/infrastructure/halls/:id/containers", async (req, res) => {
    try {
      ...
      const containers = Array.from({ length: containerCount }, (_, i) => {
        ...
        const seedValue = containerId * 1234;
        const seededRandom = (seed: number): number => {
          const x = Math.sin(seed) * 10000;
          return x - Math.floor(x);
        };
        ...
        return {
          id: containerId,
          hallId: hallId,
          hallName: `${stationName} - ${hallName}`,
          name: `Tank ${i + 1}`,
          type: "land-based",
          capacity: Math.round(seededRandom(seedValue + 3) * 1500 + 500),
          currentOccupancy: Math.round(seededRandom(seedValue + 4) * 1200 + 400),
          lastFeeding: new Date(Date.now() - seededRandom(seedValue + 5) * 8 * 60 * 60 * 1000).toISOString(),
          temperature: Math.round((seededRandom(seedValue + 6) * 2 + 8) * 10) / 10,
          oxygen: Math.round((seededRandom(seedValue + 7) * 0.5 + 7.5) * 10) / 10,
          ph: Math.round((seededRandom(seedValue + 8) * 0.3 + 7.8) * 10) / 10,
          feedType: ["Starter", "Grower", "Finisher"][Math.floor(seededRandom(seedValue + 9) * 3)],
          status: seededRandom(seedValue + 10) > 0.9 ? "maintenance" : "active"
        };
      });
